...
public final class loginsave_jsp extends org.apache.jasper.runtime.HttpJspBase
...
String str=request.getParameter("radiobutton");
   String user=request.getParameter("user");
   String passwd=request.getParameter("passwd");
   String sql;
   if(str.equals("gl"))
 { sql ="select * from admin where user='"+user+"' and passwd='"+passwd+"'";}
   else {
  sql ="select * from momber where user='"+user+"' and passwd='"+passwd+"'";}
   rs=openrs( stat, sql);
  if(rs.next()){
      session.setAttribute("loginDemo",user);
      session.setAttribute("loginbuffer","true");
	  if(str.equals("gl"))
	   response.sendRedirect("AD/ad_index.jsp");
	   else response.sendRedirect("AD/ad_index2.jsp");
 }
else {
session.setAttribute("error","请核对你的身份或检查用户名或密码是否正确");
 session.setAttribute("loginbuffer","false");
